...
% Finding Point of Contraflexure (if bending moment changes sign)
...
for i = 2:(length(x)-1) % Start from the second index and avoid the last index
    if (M(i) >= 0 && M(i+1) < 0) || (M(i) <= 0 && M(i+1) > 0) % Check for sign change
        pointOfContraflexure = x(i); % Record the position of sign change
        break; % Break the loop when the first sign change is detected
    end
end
